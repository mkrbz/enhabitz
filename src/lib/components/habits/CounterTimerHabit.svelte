<script lang="ts">
    ...
    const done = $derived(habit.currentRound >= habit.rounds);
    const roundProgress = $derived(
        Math.round((habit.roundSecondsElapsed / habit.secondsPerRound) * 100),
    );

    $effect(() => {
        if (!habit.isRunning) return;
        const id = setInterval(() => tickCounterTimer(habit.id), 1000);
    ...
    <div class="flex items-center gap-2">
        ...
        >
            <span class={done ? "text-muted-foreground" : ""}
                >{habit.currentRound}</span
            >
            <span class="text-muted-foreground">/{habit.rounds}r</span>
            <span class="text-muted-foreground mx-1">·</span>
            <span class={done ? "text-muted-foreground" : ""}
                >{formatTime(habit.roundSecondsElapsed)}</span
            >
            <span class="text-muted-foreground"
                >/{formatTime(habit.secondsPerRound)}</span
            >
        </button>
    ...
    <Dialog.Content class="max-w-sm">
        ...
        <div class="flex flex-col items-center gap-6 py-4">
                ...
                {#each { length: habit.rounds } as _, i}
                    <button
                        class={`h-8 w-8 rounded-full text-sm font-semibold transition-colors
                            ${
                                i < habit.currentRound
                                    ? "bg-primary text-primary-foreground"
                                    : i === habit.currentRound && !done
            ...
            <div class="text-6xl font-mono tabular-nums font-bold">
                {formatTime(habit.roundSecondsElapsed)}
            </div>
            <div class="text-sm text-muted-foreground">
                of {formatTime(habit.secondsPerRound)}
            </div>
            ...
            <div class="flex gap-3">
                <Button
                    variant="outline"
                    size="icon"
                    onclick={() => resetCounterTimer(habit.id)}
                >
